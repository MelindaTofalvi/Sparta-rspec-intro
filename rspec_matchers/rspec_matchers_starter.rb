...
describe "show the depths of rspec" do
  ...
  context "work through the different types of Rspec matchers" do
    ...
    it 'should test the various different type of comparison matcher' do
      ...
      # expect(e).to satisfy{e > d} # check if e is greater than d
      # expect(e).to satisfy{e <= f} # check to see if equal to or less than f
      # expect(f).to satisfy{f >= e} # check to see if f is greater than or equal to e
      # expect(e).to satisfy{d < e && e < f} # check whether e is between d & f
      # expect(d).to satisfy{ !(e < d && d < f)} # check whether d is NOT between e & f
      end
